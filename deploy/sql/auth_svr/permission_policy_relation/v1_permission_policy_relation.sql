...
-- 权限-策略关系表
create table if not exists permission_policy_relation(
    permission_code varchar(100) not null comment '权限唯一标识符',
    ...
    primary key (permission_code,policy_code),
    index permission_policy_relation_status(permission_code, is_deleted,status),
    constraint permission_policy_relation_chk_status check ( status in (0,1) )
