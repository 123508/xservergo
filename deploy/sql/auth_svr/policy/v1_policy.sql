...
create table if not exists policy (
    id  binary(16) not null  comment '策略id',
    code varchar(100) not null  comment '策略唯一标识符',
    name varchar(100) not null  default '' comment '策略名称',
    description varchar(255) not null default '' comment '策略详细描述',
    status tinyint(1) not null default 1 comment '策略是否启用:0不启用 1启用',
   ...
   primary key (id) comment '主键',
    ...
    -- 唯一约束优化
    unique index policy_udx_code (code,deleted_at_fixed) comment '保证权限标识符唯一存在',

    index policy_idx_code_deleted_status(code,is_deleted,status)
