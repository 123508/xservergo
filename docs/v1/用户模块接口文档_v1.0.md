[用户服务] - [1.0]

最后更新：2025-6-25

协议类型：gRPC

序列化协议：Protobuf

负责人：西宫炎





#### 用户服务表结构



​	1.用户表（users）

| id                                   | username | nickname |      eamil       |    phone    | gender |   avatar    | status |
| ------------------------------------ | :------: | :------: | :--------------: | :---------: | :----: | :---------: | :----: |
| 0197f8c2-152c-7376-b083-26fda360df18 | zhangsan |   张三   | 123456789@qq.com |  123456789  |   0    | https://... |   0    |
| 019801d7-e71a-7457-90b3-4a9013890296 |   lisi   |   李四   | 321645987@qq.com | 13025468452 |   1    | https://... |   0    |
| 01980954-182e-7124-9a58-3db3bec3e203 |  wangwu  |   王五   | 987654321@qq.com | 98765432110 |   0    |  https://   |   1    |



​	2.用户密码表(user_login)

|               user_id                |                           password                           |
| :----------------------------------: | :----------------------------------------------------------: |
| 0197f8c2-152c-7376-b083-26fda360df18 |                        1234567891011                         |
| 019801d7-e71a-7457-90b3-4a9013890296 |                            321654                            |
| 01980954-182e-7124-9a58-3db3bec3e203 | 8d969eef6ecad3c29a3a629280e686cf0c3f5d5a86aff3ca12020c923adc6c92 |



​	3.用户登录日志表(user_login_log)

|  id  | user_id | login_type | login_status | fail_reason | login_ip | user_agent | device |
| :--: | :-----: | :--------: | :----------: | :---------: | :------: | :--------: | :----: |
|      |         |            |              |             |          |            |        |
|      |         |            |              |             |          |            |        |
|      |         |            |              |             |          |            |        |




#### 用户服务方法列表

```protobuf
syntax = "proto3";

service UserService {
  // ---------- 认证模块 ---------- //
  rpc Register(RegisterReq) returns (OperationResult) {}                    // 用户注册
  rpc EmailLogin(EmailLoginReq) returns (LoginResp) {}                      // 邮箱登录
  rpc PhoneLogin(PhoneLoginReq) returns (LoginResp) {}                      // 手机密码登录
  rpc AccountLogin(AccountLoginReq) returns (LoginResp) {}                  // 账号登录
  rpc SmsLogin(SmsLoginReq) returns (SmsLoginResp) {}                       // 验证码登录
  rpc QrCodeLogin(QrCodeLoginReq) returns (QrCodeLoginResp) {}              // 扫码登录
  rpc ConfirmQrLogin(ConfirmQrLoginReq) returns (LoginResp) {}              // 确认扫码登录
  rpc OAuthLogin(OAuthLoginReq) returns (LoginResp) {}                      // 第三方登录
  rpc Logout(LogoutReq) returns (OperationResult) {}                        // 登出
  rpc SessionCheck(SessionCheckReq) returns (SessionStatusResp) {}          // 会话检查

  // ---------- 密码管理 ---------- //
  rpc ChangePassword(ChangePasswordReq) returns (OperationResult) {}        // 修改密码
  rpc ForgotPassword(ForgotPasswordReq) returns (OperationResult) {}        // 忘记密码
  rpc ResetPassword(ResetPasswordReq) returns (OperationResult) {}          // 重置密码

  // ---------- 绑定管理 ---------- //
  rpc StartBindEmail(StartBindEmailReq) returns (OperationResult) {}        // 预绑定邮箱
  rpc CompleteBindEmail(CompleteBindEmailReq) returns (OperationResult) {}  // 完成邮箱绑定
  rpc StartUnbindEmail(StartUnbindEmailReq) returns (OperationResult) {}    // 预解绑邮箱
  rpc CompleteUnbindEmail(CompleteUnbindEmailReq) returns (OperationResult) {} // 完成邮箱解绑
  rpc StartBindPhone(StartBindPhoneReq) returns (OperationResult) {}        // 预绑定手机
  rpc CompleteBindPhone(CompleteBindPhoneReq) returns (OperationResult) {}  // 完成手机绑定
  rpc StartChangePhone(StartChangePhoneReq)returns(OperationResult){}       // 预换绑手机
  rpc VerifyNewPhone(VerifyNewPhoneReq)returns(OperationResult){}           // 验证新手机号码
  rpc CompleteChangePhone(CompleteChangePhoneReq)returns(OperationResult){} // 完成换绑手机

  // ---------- 用户管理 ---------- //
  rpc GetUserInfo(GetUserInfoReq) returns (UserInfoResp) {}                // 获取用户信息
  rpc UpdateUserInfo(UpdateUserInfoReq) returns (OperationResult) {}       // 更新用户信息
  rpc ListUsers(ListUsersReq) returns (ListUsersResp) {}                   // 用户列表
  rpc SearchUserByNickname(SearchUserByNicknameReq) returns (SearchUserByNicknameResp){} // 根据用户昵称模糊查询

  // ---------- 账户状态 ---------- //
  rpc StartDeactivateUser(StartDeactivateReq) returns (OperationResult) {} // 预冻结用户
  rpc DeactivateUser(DeactivateUserReq) returns (OperationResult) {}       // 冻结用户
  rpc ReactivateUser(ReactivateUserReq) returns (OperationResult) {}       // 解冻用户
  rpc StartDeleteUser(StartDeleteReq) returns (OperationResult) {}         // 预删除用户
  rpc DeleteUser(DeleteUserReq) returns (OperationResult) {}               // 删除用户

  // ---------- 安全服务 ---------- //
  rpc VerifySecurityCode(VerifyCodeReq) returns (VerifyCodeResp) {}        // 验证安全码
  rpc SendVerification(SendVerificationReq) returns (OperationResult) {}   // 发送验证码
}
```



##### 公共参数部分定义

```protobuf
message OperationResult {
  bool success = 1;		  // 业务是否成功
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  string timestamp = 5;    // 服务器时间戳(毫秒)
}

message UserInfo {
  bytes user_id = 1;        // 二进制格式的UUID (16 bytes)
  string username = 2;		// 用户名
  string nickname=3;		// 用户昵称
  string email = 4;			// 用户邮箱
  string phone = 5;			// 用户手机号
  uint64 gender = 6;         // 0-未知 1-男 2-女
  string avatar = 7;		// 用户头像
  uint64 status = 8;         // 0-正常 1-冻结
  string created_at = 9;	// 用户创建时间
  string updated_at = 10;// 用户修改时间
  bool is_deleted = 11;     // 是否已删除
}

enum Operator {
  OP_EQ = 0;      // =
  OP_NEQ = 1;     // !=
  OP_GT = 2;      // >
  OP_GTE = 3;     // >=
  OP_LT = 4;      // <
  OP_LTE = 5;     // <=
  OP_LIKE = 6;    // LIKE
  OP_IN = 7;      // IN
  OP_NOT_IN = 8;  // NOT IN
}

message SafeFilter {
  string field = 1;	//过滤字段
  Operator op = 2;	//过滤操作
  oneof value {		//过滤值
    string str_val = 3;
    uint64 int_val = 4;
    double float_val = 5;
    bool bool_val = 6;
    uint64 timestamp = 7;  // UNIX毫秒时间戳
  }
}
```



### 一、认证模块



##### 1.用户注册

```protobuf
//方法名
rpc Register(RegisterReq) returns (OperationResult) {}

//请求参数
message RegisterReq {
  string username = 1;			// 用户名(后端做合法性和唯一性校验)
  string nickname=2;			// 用户昵称(敏感词屏蔽和替换)
  string password = 3;			// 用户密码(前端配合https明文传输)
  string confirm_password=4;	// 用户密码确认
  string email = 5;				// 用户邮箱(唯一性校验)
  string phone = 6;				// 用户手机号码(唯一性校验)
  uint64 gender = 7;  			// 0-未知 1-男 2-女
}


//响应参数
message OperationResult {
  bool success = 1;		  // 操作是否成功
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



##### 2.用户邮箱登录

```protobuf
//方法名
rpc EmailLogin(EmailLoginReq) returns (LoginResp) {}

//请求参数
message EmailLoginReq {
  string email = 1;		//用户邮箱
  string password = 2;	//用户密码
}

//响应参数
message LoginResp {
  string access_token = 1;	
  string refresh_token = 2;  
  UserInfo user_info = 3;	// 用户信息
}
```



##### 3.用户手机号码登录

```protobuf
//方法名
rpc PhoneLogin(PhoneLoginReq) returns (LoginResp) {} 

//请求参数
message PhoneLoginReq {
  string phone = 1;		// 用户手机号
  string password = 2;	// 用户密码
}

//响应参数
message LoginResp {
  string access_token = 1;
  string refresh_token = 2;
  UserInfo user_info = 3;	// 用户信息
}
```



##### 4.用户账号登录

```protobuf
//方法名
rpc AccountLogin(AccountLoginReq) returns (LoginResp) {}

//请求参数
message AccountLoginReq {
  string username = 1;	// 用户账号
  string password = 2;	// 用户密码
}

//响应参数
message LoginResp {
  string access_token = 1;
  string refresh_token = 2;
  UserInfo user_info = 3;	// 用户信息
}
```



##### 5.用户验证码登录

```protobuf
//方法名
rpc SmsLogin(SmsLoginReq) returns (SmsLoginResp) {}

//请求参数
message SmsLoginReq {
  string phone = 1;		   //每次请求都要带上
  string code = 2;         // 验证阶段提供
  LoginFlowType flow = 3;  // 流程类型
  string request_id = 4;   // 验证阶段携带
}

//响应参数
message SmsLoginResp {
  oneof result {
    CodeSentInfo code_sent = 1;  // 验证码发送成功
    LoginSuccess login = 2;      // 登录成功
  }
}

enum LoginFlowType {
  FLOW_REQUEST_CODE = 0;  // 请求验证码
  FLOW_VERIFY_CODE = 1;   // 验证并登录
}
```



##### 6.用户扫码登录

```protobuf
//方法名
rpc QrCodeLogin(QrCodeLoginReq) returns (QrCodeLoginResp) {}

//请求参数
message QrCodeLoginReq {
  string session_id = 1;  // 扫码后提供
}

//响应参数
message QrCodeLoginResp {
  string qr_code_url = 1;   // 二维码图片URL
  string session_id = 2;     // 登录会话ID
  uint64 expires_at = 3;      // 过期时间戳(毫秒)
}
```



##### 7.用户扫码登录确认

```protobuf
//方法名
rpc ConfirmQrLogin(ConfirmQrLoginReq) returns (LoginResp) {}

//请求参数
message ConfirmQrLoginReq {
  string session_id = 1;	// 会话id
  bytes user_id = 2;        // 确认登录的用户
}

//响应参数
message LoginResp {
  string access_token = 1;
  string refresh_token = 2;
  UserInfo user_info = 3;	// 用户信息
}
```



##### 8.用户登出

```protobuf
//方法名
rpc Logout(LogoutReq) returns (OperationResult) {}  

//请求参数
message LogoutReq {
  bytes target_user_id = 1;
  string access_token = 2;
  string refresh_token = 3;
  bytes request_user_id= 4;
}

//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



##### 9.会话检查

```protobuf
//方法名
rpc SessionCheck(SessionCheckReq) returns (SessionStatusResp) {}

//请求参数
message SessionCheckReq {
  string access_token = 1;
  bytes request_user_id=2;
}

//响应参数
message SessionStatusResp {
  bool is_valid = 1;
  uint64 last_active = 2;  // 最后活动时间戳(毫秒)
  string device_info = 3; // 设备信息
}
```



### 二、密码管理模块



##### 10.修改密码

```protobuf
//方法名
rpc ChangePassword(ChangePasswordReq) returns (OperationResult) {}

//请求参数
message ChangePasswordReq {
  bytes target_user_id = 1;
  string old_password = 2;
  string confirm_password=3;
  string new_password = 4;
  bytes request_user_id=5;
}

//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



##### 11.忘记密码

```protobuf
//方法名
rpc ForgotPassword(ForgotPasswordReq) returns (OperationResult) {} 

//请求参数
message ForgotPasswordReq {
  oneof identify{
    string phone=1;    // 手机号
    string email=2;    // 邮箱
    string username=3; // 用户名
  }
}

//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



##### 12.重置密码

```protobuf
//方法名
rpc ResetPassword(ResetPasswordReq) returns (OperationResult) {}

//请求参数
message ResetPasswordReq {
  string verification_token = 1;  // 验证码
  string new_password = 2;
  string request_id=3;
  bytes request_user_id=4;
}

//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



### 三、绑定管理模块



##### 13.预绑定邮箱

```protobuf
//方法名
rpc StartBindEmail(StartBindEmailReq) returns (OperationResult) {}

//请求参数
message StartBindEmailReq {
  bytes target_user_id = 1;
  string new_email = 2;
  bytes request_user_id=3;
}

//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



##### 14.完成邮箱绑定



```protobuf
//方法名
rpc CompleteBindEmail(CompleteBindEmailReq) returns (OperationResult) {}

//请求参数
message CompleteBindEmailReq {
  bytes target_user_id = 1;
  string new_email = 2;
  string verification_code = 3;
  string request_id=4;
  bytes request_user_id=5;
}

//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



##### 15.预解绑邮箱

```protobuf
//方法名
rpc StartUnbindEmail(StartUnbindEmailReq) returns (OperationResult) {}

//请求参数
message StartUnbindEmailReq {
  bytes target_user_id = 1;
  bytes request_user_id=2;
}

//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```

##### 16.完成邮箱解绑



```protobuf
//方法名
rpc CompleteUnbindEmail(CompleteUnbindEmailReq) returns (OperationResult) {}

//请求参数
message CompleteUnbindEmailReq {
  bytes target_user_id = 1;
  string verification_code = 2;
  string request_id=3;
  bytes request_user_id=4;
}

//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



##### 17.预绑定手机



```protobuf
//方法名
rpc StartBindPhone(StartBindPhoneReq) returns (OperationResult) {}

//请求参数
message StartBindPhoneReq {
  bytes target_user_id = 1;   // 当前用户ID
  string new_phone = 2;       // 要绑定的新手机号
  bytes request_user_id=3;    // 操作者ID
}

//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



##### 18.完成手机绑定



```protobuf
//方法名
rpc CompleteBindPhone(CompleteBindPhoneReq) returns (OperationResult) {}

//请求参数
message CompleteBindPhoneReq {
  bytes target_user_id = 1;       // 当前用户ID
  string new_phone = 2;    // 要绑定的手机号
  string verification_code = 3;  // 短信验证码
  string request_id = 4;   // 预绑定返回的请求ID
  bytes request_user_id=5;
}

//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



##### 19.预换绑手机



```protobuf
//方法名
rpc StartChangePhone(StartChangePhoneReq)returns(OperationResult){} 

//请求参数
message StartChangePhoneReq{
  bytes target_user_id=1;
  string old_phone=2;
  bytes request_user_id=3;
}

//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



##### 20.验证新手机号码



```protobuf
//方法名
rpc VerifyNewPhone(VerifyNewPhoneReq)returns(OperationResult){}

//请求参数
message VerifyNewPhoneReq{
  bytes target_user_id=1;
  string new_phone=2;
  string verification_code=3;
  bytes request_user_id=4;
  string request_id=5;
}

//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



##### 21.完成换绑手机



```protobuf
//方法名
rpc CompleteChangePhone(CompleteChangePhoneReq)returns(OperationResult){}

//请求参数
message CompleteChangePhoneReq{
  bytes target_user_id=1;
  string new_phone=2;
  string verification_code=3;
  string request_id=4;
  bytes request_user_id=5;
}

//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



### 四、用户管理模块



##### 22.获取用户信息



```protobuf
//方法名
rpc GetUserInfo(GetUserInfoReq) returns (UserInfoResp) {} 

//请求参数
message GetUserInfoReq {
  oneof identifier {
    bytes target_user_id = 1;
    string username = 2;
    string email = 3;
    string phone = 4;
  }
  bytes request_user_id=5;
}

//响应参数
message UserInfoResp {
  OperationResult result = 1;
  UserInfo user_info = 2;
}
```



##### 23.更新用户信息



```protobuf
//方法名
rpc UpdateUserInfo(UpdateUserInfoReq) returns (OperationResult) {}

//请求参数
message UpdateUserInfoReq {
  bytes target_user_id = 1;
  optional string nickname = 2;  // 可选更新字段
  optional string avatar = 3;
  optional int32 gender = 4;     // 0-未知 1-男 2-女
  bytes last_updated_by = 5;
  bytes request_user_id=6;      // 操作者ID
}
//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



##### 24.用户列表



```protobuf
//方法名
rpc ListUsers(ListUsersReq) returns (ListUsersResp) {}

//请求参数
message ListUsersReq {
  uint64 page = 1;                  // 页码(从1开始)
  uint64 page_size = 2;             // 每页数量
  repeated SafeFilter filters = 3; // 过滤条件
  repeated OrderBy order_by=4;     // 排序字段
  bytes request_user_id=5;
}

message OrderBy {
  string field = 1;    // 排序字段
  string direction = 2; // "asc"/"desc"
}

//响应参数
message ListUsersResp {
  OperationResult result = 1;
  repeated UserInfo users = 2;
  uint64 total_count = 3;           	// 总记录数
  uint64 total_pages = 4;          	// 总页数
  uint64 current_page = 5;        	// 当前页
}
```



##### 25.根据用户昵称模糊查询

```protobuf
//方法名
rpc SearchUserByNickname(SearchUserByNicknameReq) returns (SearchUserByNicknameResp){}

//请求参数
message SearchUserByNicknameReq {
  string keyword = 1;      // 查询关键字，支持模糊匹配
  uint64 page = 2;          // 页码，从1开始
  uint64 page_size = 3;     // 每页数量，建议默认20
  bytes request_user_id=4;
}

//响应参数
message SearchUserByNicknameResp {
  repeated UserInfo users = 1;   // 匹配到的用户列表
  uint64 total_count = 2;         // 总数
  uint64 total_pages = 3;         // 总页数
  uint64 current_page = 4;        // 当前页
}
```



### 五、账户状态模块



##### 26.预冻结用户



```protobuf
//方法名
rpc StartDeactivateUser(StartDeactivateReq) returns (OperationResult) {}

//请求参数
message StartDeactivateReq {
  bytes target_user_id = 1;
  bytes request_user_id = 2;  // 操作者ID
}

//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



##### 27.冻结用户



```protobuf
//方法名
rpc DeactivateUser(DeactivateUserReq) returns (OperationResult) {} 

//请求参数
message DeactivateUserReq {
  bytes target_user_id = 1;
  bytes request_user_id = 2;
  string verification_code = 3;  // 二次验证码
  string request_id=4;
}

//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



##### 28.解冻用户



```protobuf
//方法名
rpc ReactivateUser(ReactivateUserReq) returns (OperationResult) {}

//请求参数
message ReactivateUserReq {
  string identity = 1;  // 邮箱或手机号
  string code = 2;      // 验证码
  optional string new_password = 3;  // 可选新密码
  bytes request_user_id = 4;
}

//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



##### 29.预删除用户



```protobuf
//方法名
rpc StartDeleteUser(StartDeleteReq) returns (OperationResult) {}

//请求参数
message StartDeleteReq {
  bytes target_user_id = 1;
  bytes request_user_id = 2;
}

//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



##### 30.删除用户



```protobuf
//方法名
rpc DeleteUser(DeleteUserReq) returns (OperationResult) {}

//请求参数
message DeleteUserReq {
  bytes target_user_id = 1;
  bytes request_user_id = 2;
  string verification_code = 3;  // 二次验证码
  string request_id=4;
}

//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



### 六、安全服务模块



##### 31.验证安全码



```protobuf
//方法名
rpc VerifySecurityCode(VerifyCodeReq) returns (VerifyCodeResp) {}

//请求参数
message VerifyCodeReq {
  string identity = 1;  // 邮箱或手机号
  string code = 2;
  CodeType code_type = 3;
}

enum CodeType {
  CODE_LOGIN = 0;
  CODE_BIND_EMAIL = 1;
  CODE_RESET_PASSWORD = 2;
  CODE_BIND_PHONE = 3;
}

//响应参数
message VerifyCodeResp {
  bool valid = 1;
  string token = 2;  // 用于后续操作
}
```



##### 32.发送验证码



```protobuf
//方法名
rpc SendVerification(SendVerificationReq) returns (OperationResult) {}

//请求参数
message SendVerificationReq {
  string identity = 1;  // 邮箱或手机号
  VerificationType type = 2;
}

enum VerificationType {
  VERIFY_SMS = 0;
  VERIFY_EMAIL = 1;
}

//响应参数
message OperationResult {
  bool success = 1;
  string code = 2;        // 业务状态码
  string message = 3;     // 用户提示信息
  string request_id = 4;  // 请求追踪ID
  uint64 timestamp = 5;    // 服务器时间戳(毫秒)
}
```



